...
export class MatmulBufferVec4Op extends BufferOp {
  ...
  constructor(
      ...
      workPerThread: [number, number, number] = [8, 8, 1]) {
    ...
    */
    ...
    vec4 mm_readA(int row, int col) {
      if (row < dimAOuter && col < dimInner) {
        vec4 resultA = A[row * dimInner + col];
        return resultA;
      } else {
    ...
    vec4 mm_readB(int row, int col) {
      if (row < aShape[2] && col < dimBOuter) {
        vec4 resultB = B[row * dimBOuter + col];
        return resultB;
      } else {
        return vec4(0.0, 0.0, 0.0, 0.0);
      }
